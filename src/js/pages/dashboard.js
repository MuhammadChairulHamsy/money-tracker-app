...
const Dashboard = {
  ...
  _initialListener() {
    ...
    recordDetailModal.addEventListener('show.bs.modal', (event) => {
      ...
      const dataRecord = this._userTransactionsHistory.find((item) => item.id == button.dataset.recordId);
  ...
  _populateTransactionsDataToCard(transactionsHistory = []) {
    let amountIncome = 0;
    let amountExpense = 0;

    transactionsHistory.forEach((item) => {
      if (item.type === 'income') amountIncome += item.amount;
      else if (item.type === 'expense') amountExpense += item.amount;
    });
  ...
  _populateTransactionsRecordToTable(transactionsHistory = []) {
    ...
    if (transactionsHistory.length <= 0) {
      ...
      recordBodyTable.innerHTML = `<tr><td colspan="${recordHeadTable.querySelectorAll('td,th').length}"><div class="text-center">Tidak ada catatan transaksi</div></td></tr>`;
      return;
    }
